...
- Caddy настроен на автоматический HTTPS для `shtq.pro` и `www.shtq.pro`; `www` перенаправляется на основной домен постоянным `301` с сохранением пути и query string. Статические файлы раздаются из активного release, а `/api/contact` и `/health` проксируются на contact API.
- Для `/_next/static/*` установлен годовой immutable-кэш. Изображения, шрифты, стили, скрипты и видео вне этого каталога кешируются на 30 дней без `immutable`; HTML остаётся с `no-cache`.
- Включены HSTS на один год и применяемая CSP. Для legacy-кейса Tornado используется отдельная CSP с разрешениями только для его Google Fonts, Tailwind CDN, Iconify и Pexels; остальные страницы получают более узкую политику с разрешениями для Яндекс Метрики.
- DNS-записи `A @` и `A www` направлены на VPS. Запись `AAAA` не создавалась.
...
- `https://www.shtq.pro/` имеет валидный TLS-сертификат и возвращает постоянный редирект на `https://shtq.pro/`.
- Повторный скан Security Headers после включения CSP и HSTS получил оценку `A`.
